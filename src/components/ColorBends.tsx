...
void main() {
  ...
  if (uColorCount > 0) {
    // Simple gradient based on colors
    float t = uv.y + sin(uv.x * uFrequency + uTime * uSpeed) * 0.1;
    vec3 color1 = uColors[0];
    vec3 color2 = uColorCount > 1 ? uColors[1] : uColors[0];
    vec3 color3 = uColorCount > 2 ? uColors[2] : uColors[0];

    col = mix(color1, color2, t);
    col = mix(col, color3, sin(uv.x * 2.0 + uTime) * 0.5 + 0.5);
  } else {
    // Fallback pattern
  ...
  gl_FragColor = vec4(col, uTransparent > 0 ? 0.8 : 1.0);
}
`;
